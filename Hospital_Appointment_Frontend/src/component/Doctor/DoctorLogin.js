...
export class DoctorLogin extends Component {
  ...
  setData1() {
    ...
    axios.get("http://localhost:8102/doctorlogin1/" + username1).then((res) => {
      console.log(res);

      console.log(res.data.type);
      localStorage.setItem("LoginDoctortype", res.data.type);
      this.setState({
        users: res.data,
        username: "",
        password: "",
  ...
  submit(event, name) {
    ...
    axios
      .post("http://localhost:8102/doctorlogin/", {
      ...
      .then((res) => {
        ...
        if (res.data == 0) {
          this.props.history.push("/doctormain");
        } else {
  ...
  setData() {
    localStorage.setItem("LoginDoctorname", this.state.username);
  }
