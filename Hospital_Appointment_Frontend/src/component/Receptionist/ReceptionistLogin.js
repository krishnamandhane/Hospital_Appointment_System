...
export class ReceptionistLogin extends Component {
  ...
  componentDidMount() {
    axios.get("http://localhost:8102/receptionistlogin/").then((res) => {
      this.setState({
        users: res.data,
        username: "",
        password: "",
  ...
  submit(event, name) {
    ...
    axios
      .post("http://localhost:8102/receptionistlogin/", {
      ...
      .then((res) => {
        ...
        if (res.data == 0) {
          this.props.history.push("/receptionistmain");
        } else {
  ...
  render() {
    return (
      <div style={{ marginBottom: "70px" }}>
        ...
        <h1 class="text-center">Receptionist Login</h1>
