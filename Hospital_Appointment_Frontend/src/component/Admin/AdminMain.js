...
export class AdminMain extends Component {
    ...
    componentDidMount() {
        axios.get("http://localhost:8102/loadappointment/")
            .then((res) => {
                this.setState({
    ...
    edit1(id){
        axios.get(`http://localhost:8102/updatebyid/${id}`)
        .then((res)=>{
    ...
    delete(id) {
        axios.delete(`http://localhost:8102/delappointment/${id}`)
            .then(() => {
    ...
    submit(event,id) {
        ...
        axios.put("http://localhost:8102/edit", {
                            ...
                            this.state.appointment.map(user =>
                                <tr key={user.id}>
                                    ...
                                    <td>
                                        ...
                                        <button onClick={(e) => this.edit1(user.id)} type="button" className="btn btn-primary" name="action">Edit</button>
                                    </td>
                                    <td>
                ...
                <form onSubmit={(e) => this.submit(e, this.state.id)} method="POST">
                    ...
                    <div className="form-group mt-3">
                        <div>
                            ...
                            <input onChange={(e) => this.setState({ fname: e.target.value })} value={this.state.fname} type="text" class="form-control" id="inputEmail4" />
                        </div>
                    </div>
                    <div className="form-group mt-3">
                        <div>
                            <label for="inputPassword4">Email</label>
                            <input onChange={(e) => this.setState({ email: e.target.value })} value={this.state.email} type="text" class="form-control" id="inputPassword4" />
                        </div>
                    </div>
                    <div className="form-group mt-3">
                        <label for="inputAddress">Address</label>
                        <input onChange={(e) => this.setState({ address: e.target.value })} value={this.state.address} type="text" class="form-control" id="inputAddress" />
                    </div>
                    <div className="form-group mt-3">
                        <div>
                            <label for="inputCity">City</label>
                            <select onChange={(e) => this.setState({ city: e.target.value })} value={this.state.city} id="inputState" class="form-control">
                    ...
                    <div className="form-group mt-3">
                        <div className="form-group">
                            ...
                            <select onChange={(e) => this.setState({ type: e.target.value })} value={this.state.type} id="inputState" class="form-control">
                                <option selected>Choose...</option>
                        ...
                        <div className="form-group mt-3">
                            ...
                            <input onChange={(e) => this.setState({ timeofapp: e.target.value })} value={this.state.timeofapp} type="time" id="inputMDEx1" class="form-control" />
                        </div>                      
                        <div className="form-group mt-3">
                            <label for="inputState">Enter date</label>
                            <input onChange={(e) => this.setState({ dateofapp: e.target.value })} value={this.state.dateofapp} type="date" class="form-control" id="inputAddress" />
                        </div>
                    </div>
                    <div className="form-group mt-3">
                        <button type="submit" name="action" className="btn btn-primary">Save Appointment</button>
                    </div>
                </form>
